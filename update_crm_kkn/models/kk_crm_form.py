...
class UpdateCrmForm(models.Model):
    ...
    sale_type = fields.Many2one(comodel_name='kk.sale.type', string = 'Sale Type' , required = True )
    branch_type = fields.Many2one(comodel_name= 'kk.branch.type', string = 'Branch Type', required= True)
    source = fields.Many2one(comodel_name= 'kk.lead.source' , string = 'Branch', required= True)
    free_of_cost = fields.Selection( selection= [('YES','YES'),('NO','NO')], string = 'FOC', help='Free Of Cost (FOC)', required = True)
    redundant_link = fields.Selection( selection= [('YES','YES'),('NO','NO')], string = 'Redundant Link', help='Backup link incase one link goes down', required = True)
    ...
    expected_revenue_mrc = fields.Monetary(string='MRC', help='Expected Revenue (MRC)', currency_field="currency_id")
    taxes = fields.Many2one(comodel_name='account.tax')
    one_time_cost_otc = fields.Monetary(string='OTC', help='One Time Cost (OTC)' , currency_field="currency_id")
    ...
    approx_won_date = fields.Date(string='Approx. Won Date', help= 'Till when you can make this lead to reach won stage' )
    lead_company = fields.Many2one(comodel_name='res.company', string='Lead Company')
    requested_medium = fields.Many2one(comodel_name='kk.requested.medium',string='Requested Medium')
    approved_medium = fields.Many2one(comodel_name='kk.requested.medium',string='Approved Medium')
